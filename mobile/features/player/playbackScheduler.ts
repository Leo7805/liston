...
import type { PlaybackMode } from '@/features/player/player.types';
...
/** Get the play count for a sentence */
...
function sortByMostPlayed(sentences: SentenceItem[]): SentenceItem[] {
  return [...sentences].sort((a, b) => getPlayCount(b) - getPlayCount(a));
}
...
function createLeastPlayedFirstQueue(
...
export function createPlaybackQueue(
  ...
  const playableSentences = sentences.filter(
    (sentence) => sentence.original.trim() || sentence.translation.trim()
  );
